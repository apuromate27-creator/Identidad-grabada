...
export default function CategoryPage({params}:{params:{category:string}}){if(!categories.some(c=>c.id===params.category))notFound();return <main className="min-h-screen bg-[#21150e] text-white"><Header/><section className="max-w-7xl mx-auto px-6 py-20"><Title eyebrow="Categoría" title={`Mates ${getCategoryLabel(params.category)}`} text="Elegí el modelo ideal y personalizalo con nombre, frase, iniciales o logo."/><ProductCatalog initialCategory={params.category}/></section><Footer/><WhatsAppFloat/></main>}
