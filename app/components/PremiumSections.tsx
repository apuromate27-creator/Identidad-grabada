    ...
    <>
      ...
      <section className="border-y border-white/10 bg-[#0b0b0b] py-24">
        <div className="max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center">
          <div className="min-h-[560px] rounded-[3rem] overflow-hidden border border-white/10 bg-black"><ImageSlot title="Proceso de grabado láser" /></div>
      ...
      <section className="bg-[#0b0b0b] border-y border-white/10 py-24">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="overflow-x-auto mt-12 border border-white/10 rounded-[2rem]">
            <table className="w-full min-w-[760px] text-left">
              <thead className="bg-black/60"><tr>{["Modelo","Material","Tamaño","Ideal para","Personalización"].map(x=><th key={x} className="px-6 py-5 text-[#d6b17a]">{x}</th>)}</tr></thead>
      ...
      <section className="max-w-7xl mx-auto px-6 py-24">
        <div className="grid lg:grid-cols-2 gap-8">
          <div className="bg-[#b68b52] text-black rounded-[3rem] p-10 md:p-14">
            ...
            <Link href="/empresas" className="inline-block mt-8 bg-black text-white px-8 py-5 rounded-2xl font-black">Ver propuestas para empresas</Link>
          </div>
          <div className="bg-[#0b0b0b] border border-white/10 rounded-[3rem] p-10 md:p-14">
...
function Step({n,title,text}:{n:string;title:string;text:string}) {
  return <div className="grid grid-cols-[64px_1fr] gap-4 border border-white/10 rounded-2xl p-5 bg-black/30"><div className="w-12 h-12 rounded-full bg-[#b68b52] flex items-center justify-center font-black">{n}</div><div><h3 className="font-black text-xl">{title}</h3><p className="text-zinc-500 mt-1">{text}</p></div></div>;
}
function Material({title,text,items}:{title:string;text:string;items:string[]}) {
  return <div className="bg-[#0b0b0b] border border-white/10 rounded-[3rem] overflow-hidden"><div className="h-80"><ImageSlot title={title} /></div><div className="p-8"><h3 className="text-4xl font-black">{title}</h3><p className="text-zinc-400 text-lg mt-4">{text}</p><div className="grid gap-3 mt-6">{items.map(i=><p key={i} className="text-zinc-300"><span className="text-[#d6b17a] mr-3">✓</span>{i}</p>)}</div></div></div>;
}
function Row({model,material,size,use}:{model:string;material:string;size:string;use:string}) {
  return <tr className="bg-[#0b0b0b]"><td className="px-6 py-5 font-black">{model}</td><td className="px-6 py-5 text-zinc-400">{material}</td><td className="px-6 py-5 text-zinc-400">{size}</td><td className="px-6 py-5 text-zinc-400">{use}</td><td className="px-6 py-5 text-[#d6b17a]">Disponible</td></tr>;
}
