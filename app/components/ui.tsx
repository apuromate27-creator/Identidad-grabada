...
export function Header({overlay=false}:{overlay?:boolean}){
  ...
  return <>
    <header className={`${overlay?"fixed":"sticky"} inset-x-0 top-0 z-[70] border-b transition-all duration-500 ${transparent?"border-white/10 bg-gradient-to-b from-[#120c08]/72 to-transparent py-3":"border-[#d6b17a]/15 bg-[#1d130d]/45/88 py-1.5 shadow-[0_18px_55px_rgba(0,0,0,.32)] backdrop-blur-xl"}`}>
    ...
    <div className={`fixed inset-0 z-[90] xl:hidden transition-opacity duration-300 ${menuOpen?"opacity-100 pointer-events-auto":"opacity-0 pointer-events-none"}`}>
      ...
      <aside className={`absolute right-0 top-0 h-full w-[88%] max-w-sm bg-[#1d130d]/45 border-l border-[#d6b17a]/20 p-6 shadow-2xl transition-transform duration-300 ${menuOpen?"translate-x-0":"translate-x-full"}`}>
